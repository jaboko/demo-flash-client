...
package com.junkbyte.console.view
{
    ...
    public class MainPanel extends ConsolePanel
    {
        ...
        public function MainPanel(m:Console)
        {
            ...
            console.cl.addCLCmd("filter", setFilterText,
                    "Filter console logs to matching string. When done, click on the * (global channel) at top.", true);
        ...
        private function makeLine(line:Log, showch:Boolean):String
        {
            ...
            if (_filterRegExp)
            {
                ...
                while (result != null)
                {
                    ...
                    {
                        txt = txt.substring(0, index) + "<u>" + txt.substring(index, index + match.length) + "</u>" +
                              txt.substring(index + match.length);
            ...
            } else if (_filterText)
            {
                ...
                while (index >= 0)
                {
                    txt = txt.substring(0, index) + "<u>" + txt.substring(index, index + _filterText.length) + "</u>" +
                          txt.substring(index + _filterText.length);
        ...
        public function getChannelsLink(limited:Boolean = false):String
        {
            ...
            for (var i:int = 0; i < len; i++)
            {
                ...
                var channelTxt:String = ((!filtering && i == 0) || (filtering && i != 0 && chShouldShow(channel))) ?
                "<ch><b>" + channel + "</b></ch>" : channel;
        ...
        private function linkHandler(e:TextEvent):void
        {
            ...
            {
                console.report("A new window should open in browser. If not, try searching for 'Flash Player Global Security Settings panel' online :)",
                        -1);
