        ...
        public function inspect(obj:*, viewAll:Boolean = true, ch:String = null):void
        {
            ...
            if (nodes.length())
            {
                for each (var metadataX:XML in nodes)
                {
                    ...
                    if (refIndex) props.push("<a href='event:cl_" + refIndex + "_dispatchEvent(new " + et + "(\"" + en + "\"))'>" + en +
                                             "</a><p0>(" + et + ")</p0>");
            ...
            {
                report(" const <p3>" + constantX.@name + "</p3>:" + constantX.@type + " = " + makeValue(cls, constantX.@name.toString()) +
                       "</p0>", 1, true, ch);
            }
            if (nodes.length())
            {
            ...
            for each (var accessorX:XML in nodes)
            {
                if (viewAll || self == accessorX.@declaredBy)
                {
                    ...
                    if (refIndex && (isstatic || !isClass))
                    {
                        str += " <a href='event:cl_" + refIndex + "_" + accessorX.@name + "'><p3>" + accessorX.@name + "</p3></a>:" +
                               accessorX.@type;
                    } else
                    {
