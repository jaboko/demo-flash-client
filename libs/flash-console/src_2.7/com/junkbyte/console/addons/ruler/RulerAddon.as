 ...
 *
 */
package com.junkbyte.console.addons.ruler
{
    ...
    public class RulerAddon extends Sprite
    {
        ...
        public static function addToMenu(menuName:String = "ruler", console:Console = null):void
        {
            ...
            if (console)
            {
                console.addMenu(menuName, startRuler, new Array(console),
                        "Screen Ruler::Measure the distance and angle between two points on screen.");
            }
        }

        private static function startRuler(console:Console):void
        {
            var ruler:RulerAddon = new RulerAddon(console);
            ruler.start();
        }
        ...
        public function RulerAddon(console:Console)
        {
            _console = console;
            buttonMode = true;
            _points = new Array();
            ...
            p = globalToLocal(p);
            _area = new Rectangle(-console.stage.stageWidth * 1.5 + p.x, -console.stage.stageHeight * 1.5 + p.y,
                    console.stage.stageWidth * 3, console.stage.stageHeight * 3);
            graphics.beginFill(config.style.backgroundColor, 0.2);
            graphics.drawRect(_area.x, _area.y, _area.width, _area.height);
            graphics.endFill();
            //
            _posTxt = _console.panels.mainPanel.makeTF("positionText", true);
            ...
            onMouseMove();
            if (hideMouseWhenActive)
            {
                Mouse.hide();
            }
            _console.report("<b>Ruler started. Click on two locations to measure.</b>", -1);
        }

        public function start():void
        {
            _console.addChild(this);
        }

        private function get config():ConsoleConfig
        {
            return _console.config;
        }
        ...
        private function onMouseClick(e:MouseEvent):void
        {
            e.stopPropagation();
            var p:Point;
            var style:ConsoleStyle = config.style;
            if (_points.length == 0)
            {
            ...
            else if (_points.length == 1)
            {
                ...
                //
                _console.report("Ruler results: (red) <b>[" + p.x + "," + p.y + "]</b> to (orange) <b>[" + p2.x + "," + p2.y + "]</b>", -2);
                _console.report("Distance: <b>" + round(d, 100) + "</b>", -2);
                _console.report("Mid point: <b>[" + mp.x + "," + mp.y + "]</b>", -2);
                _console.report("Width:<b>" + w + "</b>, Height: <b>" + h + "</b>", -2);
                _console.report("Angle from first point (red): <b>" + a1 + "°</b>", -2);
                _console.report("Angle from second point (orange): <b>" + a2 + "°</b>", -2);
            }
            else
            {
                exit();
        ...
        {
            var format:TextFormat = new TextFormat(config.style.menuFont, config.style.menuFontSize, col, b, true, null, null,
