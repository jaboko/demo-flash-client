    ...
    public class DisplayMapPanel extends ConsolePanel
    {
        ...
        private function printChild(display:DisplayObject, currentStep:uint):String
        {
            ...
            if (display is DisplayObjectContainer)
            {
                ...
                {
                    string = "<p5><b>" + generateSteps(display, currentStep) + makeLink(display, "-" + container.numChildren, "minimize") +
                             "</b> " + makeName(display) + "</p5><br/>";
                    string += printChildren(container, currentStep + 1);
                }
                else
                {
                    string = "<p4><b>" + generateSteps(display, currentStep) + makeLink(display, "+" + container.numChildren, "expand") +
                             "</b> " + makeName(display) + "</p4><br/>";
                }
                return string;
            }
        ...
        {
            return makeLink(display, display.name, "scope") + " {<menu>" +
                   makeLink(display, LogReferences.ShortClassName(display), "inspect") + "</menu>}";
        }
